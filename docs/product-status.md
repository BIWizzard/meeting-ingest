...
- semantic judgment is a versioned prompt contract, `semantic_guidance_version` `1.1`, carried in the request alongside the grounding index, with the four maintained provider instruction surfaces aligned to it; `1.1` shipped in build `meeting-ingest-0.2.0-g3695fc350c77-s61e1660c5dc8`, which the pinned consumer now runs, and its only acceptance evidence is a development-evidence run, since release-evidence acceptance on that frozen build has not been run;
...
Guidance 1.1 answers that finding and is released. Its acceptance run on 2026-07-29 passed 18/18 blocking assertions and 1/1 advisory with both required reviews concordant, closing both 1.0 failures and recording four framing-restraint quality findings; it also ran on an editable checkout under `--development-override` and is development/non-release evidence. See `docs/sessions/2026-07-29-guidance-1_1-semantic-acceptance-dev-run.md`. The slice was released as build `meeting-ingest-0.2.0-g3695fc350c77-s61e1660c5dc8` (receipt cut from commit `3695fc3`, published, installed, and pinned per the release flow), and the HTV consumer now runs and is pinned to it. See `docs/sessions/2026-07-29-guidance-1_1-release.md`.

Release-evidence acceptance on the frozen 0.2.0 build has not been run. Releasing 1.1 is a distribution fact, not acceptance evidence, so milestone-proof semantic acceptance on the approved runtime is not achieved.
