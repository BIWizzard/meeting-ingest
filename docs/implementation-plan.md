...
**Status:** implemented, released, and accepted on release evidence with a scoped claim. `semantic_guidance_version` `1.0` shipped as build `g51ff17173bea` and failed two release-evidence acceptance runs on a wording gap; the `1.1` revision answering that finding is released as build `meeting-ingest-0.2.0-g3695fc350c77-s61e1660c5dc8`, installed and pinned, and release-evidence acceptance on that frozen build passed 18/18 blocking and 1/1 advisory on 2026-07-29. Rules 4 and 5 are confirmed against the assertions that caused the 1.0 failures; rule 6 is falsified rather than unproven and is excluded from the claim, with fixture detection for it still to be written. The engine indexes normalized transcript speaker labels and timestamps, binds that index into every provider request, recomputes the index from the persisted transcript rather than trusting request metadata, and rejects ungrounded attendee raw labels, evidence speakers, and evidence timestamps before signals, markdown, ledger success snapshots, archive, reconcile, or cache cleanup. `semantic_guidance_version` `1.1` binds only for providers that make semantic judgments — the Anthropic adapter and the session path; the non-semantic mock provider binds `none`, as the frozen contract requires. The four maintained extraction instruction surfaces carry the same versioned rules.
