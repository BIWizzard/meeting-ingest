...
**Status:** implemented and released, with acceptance so far recorded only as development evidence. `semantic_guidance_version` `1.0` shipped as build `g51ff17173bea` and failed two release-evidence acceptance runs on a wording gap; the `1.1` revision answering that finding is released as build `meeting-ingest-0.2.0-g3695fc350c77-s61e1660c5dc8`, installed and pinned, and release-evidence acceptance on that frozen build has not been run. The engine indexes normalized transcript speaker labels and timestamps, binds that index into every provider request, recomputes the index from the persisted transcript rather than trusting request metadata, and rejects ungrounded attendee raw labels, evidence speakers, and evidence timestamps before signals, markdown, ledger success snapshots, archive, reconcile, or cache cleanup. `semantic_guidance_version` `1.1` binds only for providers that make semantic judgments — the Anthropic adapter and the session path; the non-semantic mock provider binds `none`, as the frozen contract requires. The four maintained extraction instruction surfaces carry the same versioned rules.
