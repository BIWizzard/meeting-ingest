...
**Status:** implemented, with the acceptance run so far recorded only as development evidence and conducted under `semantic_guidance_version` `1.0`; the `1.1` revision now in committed `main` has no acceptance evidence of its own. The engine indexes normalized transcript speaker labels and timestamps, binds that index into every provider request, recomputes the index from the persisted transcript rather than trusting request metadata, and rejects ungrounded attendee raw labels, evidence speakers, and evidence timestamps before signals, markdown, ledger success snapshots, archive, reconcile, or cache cleanup. `semantic_guidance_version` `1.1` binds only for providers that make semantic judgments — the Anthropic adapter and the session path; the non-semantic mock provider binds `none`, as the frozen contract requires. The four maintained extraction instruction surfaces carry the same versioned rules.
