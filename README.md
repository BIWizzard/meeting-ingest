...
Transcript grounding is enforced by the engine on every provider path, and the approved installed build carries it. Attendee raw labels and communication-signal evidence speakers and timestamps must copy the normalized transcript verbatim, and an ungrounded or tampered response is rejected before any durable primary output exists: validation runs ahead of signal writes, markdown writes, ledger success snapshots, archive, reconcile, and cache cleanup, and the only durable record a failure writes is the `ingest_failed` snapshot. Semantic judgment is governed by a versioned guidance contract, `semantic_guidance_version` `1.1`, carried with the grounding index in each provider request that makes semantic judgments and recorded in durable provenance.

This guards newly ingested output. It is not a claim of general semantic correctness, and it does not adopt, correct, or mutate existing artifacts. The acceptance run recorded on 2026-07-26 passed all 18 blocking assertions under guidance 1.0 with concordant human and independent blind review, but it ran on an editable checkout under `--development-override` and is development/non-release evidence; two release-evidence attempts on build `g51ff17173bea` then failed semantic assertions with a consistent `semantic_guidance` 1.0 wording gap — recorded as a contract finding, with the deterministic gates passing on the frozen build in both attempts.

The guidance 1.1 revision answering that finding is released as build `meeting-ingest-0.2.0-g3695fc350c77-s61e1660c5dc8`, installed and pinned. Release-evidence acceptance on that frozen build passed 18/18 blocking and 1/1 advisory on 2026-07-29 under clean approved-runtime conditions, with concordant human semantic and independent blind review, and is accepted as milestone proof with a scoped claim: rules 4 and 5 are confirmed against the two assertions that sank both 1.0 release-evidence runs, while rule 6, the framing-restraint rule 1.1 added, is **falsified rather than unproven** — the run widened a committed action item beyond the scope its owner accepted, caught by blind review because no assertion detects it. Rule 6 detection is open fixture work. An earlier attempt the same day scored 18/18 but was rejected as milestone proof because the host had cached the pre-release agent definition. See `docs/testing/semantic-integrity-acceptance.md`, `docs/sessions/2026-07-26-task7-release-and-acceptance.md`, `docs/sessions/2026-07-29-guidance-1_1-semantic-acceptance-dev-run.md`, `docs/sessions/2026-07-29-guidance-1_1-release.md`, and `docs/sessions/2026-07-29-guidance-1_1-release-evidence-acceptance.md`.
