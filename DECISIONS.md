...
### 33. Meeting-extraction pin raises to claude-opus-5 on controlled-comparison evidence (2026-07-24)

Claude Opus 5 released 2026-07-24 at exact cost parity with the incumbent Opus 4.8 ($5/$25 per MTok). The Decision 32 controlled comparison was re-run with claude-opus-5 as candidate against claude-opus-4-8: the same two shipped meetings (the twelve-speaker daily standup and the short 1:1 working call), byte-identical normalized transcripts recomputed and hash-verified against each request's `normalized_transcript_sha256`, byte-identical canonical briefs from `docs/session-provider-subagent-prompt.md`, and fresh incumbent reruns so both arms shared one brief. Model identity was verified per run from the harness-reported exact model ID and recorded in `provider.model_id` (`claude-opus-4-8[1m]` / `claude-opus-5[1m]`), closing the self-report gap in the Decision 32 evidence. All four envelopes validated against the request response contracts; scratch envelopes only, zero engine writes, honoring OB-002-1.

Evidence: neither model hallucinated, and the candidate's extraction was a transcript-verified superset of the incumbent's on both meeting types. On the standup — the meeting type Decision 32 weighted most — grounded action-item recall doubled (6 to 12, every addition owner-attributed and transcript-verified), and the candidate recovered items the incumbent missed entirely: the 12–1 environment change-freeze announcement, the snapshot-versus-completion-date scope correction, the three-track validation-scope assignment, the Microsoft-issue contingency captured as an owned action rather than an ownerless question, and per-person completion percentages. On the working call the incumbent already held all Decision 32 anchor items; the candidate still added the run-status confirmation, the unverified bronze-change risk, the attention-split risk, and better-calibrated uncertainty hedging. Both arms captured both anchor directive decisions.

Decision: raise. At cost parity there is no counterweight to a no-regression recall lift that lands exactly where Decision 32 located the product's value. The meeting-extraction role's pin moves from claude-opus-4-8 to claude-opus-5. As with Decision 32, the pin edit is not made from this repo: it routes to the role registry and both agent definitions in `~/.claude` as an agent-orchestration work item, firing the global pin lint (OB-003-3). The `quality` alias remains a single fixed mapping. Privacy stance unchanged and no ruling needed: Opus 5 is an Anthropic model, transcripts stay session-side, and the session-provider path is unaffected.

**Execution addendum (2026-07-25).** The routing above was half wrong, and the Approved Runtime caught it. The agent-orchestration workstream executed the swap by editing the installed `~/.claude/agents/meeting-ingest-session-provider.md` directly — but under Record 002 that file is a receipt-managed workflow artifact whose source of truth is `docs/claude-agents/meeting-ingest-session-provider.md` in this repo. The next HTV ingest (2026-07-25, six meetings) hit the readiness gate fail-closed on the receipt hash mismatch (exit 12); the in-field remediation restored `claude-opus-4-8` per the documented procedure and the ingest proceeded validly on the incumbent tier. This is the fail-closed contract working as designed and counts as positive dogfood evidence for Record 002.

Corrected routing for the meeting-extraction pinned worker: the registry-row edit lives in `~/.claude`, but the agent-file half of the swap flows through this repo — source pin edit here, then a new reproducible approved release and receipt, owner publish to the private-alpha channel, owner install and repin at HTV. Direct edits to the installed agent file are receipt drift and will be rejected. The source pin is flipped to `claude-opus-5` with this addendum; the ruling is recorded but not effective at HTV until the next approved release lands (registry currently leads the receipt, and the global pin lint will flag the mismatch until then). Follow-ups owed elsewhere: the registry row's swap-point column should name this repo's release flow rather than a direct agent-file edit, and the HTV-side session memory instructing restoration of `claude-opus-4-8` must be retired once the new receipt blesses the Opus 5 pin.

### 34. Exact source facts fail closed; semantic judgment is prompt contract plus acceptance evidence (2026-07-25)

Dogfood review of a shipped meeting artifact found a provider adding a `(Contractor)` qualifier to a speaker label that never carried one, alongside four judgment defects: an invented meridiem on a night-context time, causal language over an unconfirmed root cause, a rejected proposal promoted into an open action, and an overconfident nickname identity merge. Those two kinds of defect are not the same problem and must not get the same control.

Exact source facts fail closed. Attendee raw labels, signal evidence speakers, signal evidence timestamps, and a persisted request's grounding metadata are all checkable against one deterministic index of the normalized transcript. A response that invents, qualifies, recases, or repunctuates any of them fails through the existing `invalid_provider_output` / `ProviderValidationError` path before markdown, signals, ledger, archive, reconcile, or handoff cleanup. Matching is case-sensitive after whitespace normalization because capitalization and parenthetical qualifiers are source provenance, not display formatting. The frozen matching rules, deterministic checks, and validation issue text are in `docs/artifact-contract.md`.

Semantic judgment is a versioned prompt contract plus acceptance evidence. Time context, epistemic certainty, proposal and action disposition, identity ambiguity, and TL;DR consistency cannot be proven from arbitrary natural language, and pretending otherwise would buy brittle heuristics that fail both ways. One versioned guidance source is bound into each provider request as `semantic_guidance_version`, echoed by the response, and stamped durably in artifact front matter, run summaries, and ledger provider provenance so a deleted handoff never erases which rules shaped the output. Conformance is evaluated by a synthetic acceptance case with machine-readable assertions plus human and independent blind review, not by runtime heuristics.

No automatic second-model review is added to the ingest path. A judge call would double extraction cost and latency for every meeting in a maintainer-only alpha, and it would move the quality bar from a reviewable versioned contract into an unreviewable one. Acceptance evaluation carries that weight instead.
