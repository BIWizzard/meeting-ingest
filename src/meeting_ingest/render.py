...
def _front_matter(response: ProviderResponse, context: RenderContext, generated_at: str) -> list[str]:
    provenance = _runtime_provenance(context)
    ledger_record_id = context.runtime_provenance_ledger_record_id
    if ledger_record_id is None:
        raise ValueError("RenderContext.runtime_provenance_ledger_record_id is required")
    return [
        "---",
        'schema_version: "1.1"',
        ...
        *([f"provider_host: {context.provider_host}"] if context.provider_host else []),
        ...
        f"runtime_provenance_sha256: {_quote(runtime_provenance_sha256(provenance))}",
        f"runtime_provenance_ledger_record_id: {ledger_record_id}",
        "runtime_provenance:",
        f"  semantic_version: {_yaml_scalar(provenance['semantic_version'])}",
        f"  build_id: {_yaml_scalar(provenance['build_id'])}",
        f"  source_commit: {_yaml_scalar(provenance['source_commit'])}",
        f"  source_tree_sha256: {_yaml_scalar(provenance['source_tree_sha256'])}",
        f"  install_mode: {_yaml_scalar(provenance['install_mode'])}",
        f"  runtime_mode: {_yaml_scalar(provenance['runtime_mode'])}",
        f"  workflow_contract_version: {_yaml_scalar(provenance['workflow_contract_version'])}",
        f"  development_override_reason: {_yaml_scalar(provenance['development_override_reason'])}",
...
def _overview(response: ProviderResponse, context: RenderContext) -> list[str]:
    provenance = _runtime_provenance(context)
    development_notice = []
    if provenance["runtime_mode"] == "development":
        development_notice = [
            f"**Development artifact:** generated by a development runtime — "
            f"{_markdown_text(str(provenance['development_override_reason']))}.",
            "",
        ]
    return [
        "## Meeting Overview",
        ...
        *development_notice,
        ...
        f"| Runtime | {_cell(provenance['runtime_mode'])} ({_cell(provenance['build_id'])}) |",
        "",
    ]
...
def _runtime_provenance(context: RenderContext) -> dict[str, Any]:
    if context.runtime_provenance is None:
        raise ValueError("RenderContext.runtime_provenance is required")
    return context.runtime_provenance
